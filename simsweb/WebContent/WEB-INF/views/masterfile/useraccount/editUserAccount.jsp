...
<body>
	...
	<div style="width: 100%">
		...
		<form:form action="/simsweb/updateUserAccount" method="post" modelAttribute="userAccount"  cssClass="form-horizontal">
		
			<form:hidden path="id"/>
			<form:hidden path="emp.id"/>
			<form:hidden path="emp.lastName"/>
			<form:hidden path="emp.firstName"/>
			<form:hidden path="emp.middleName"/>
				...
				<tr>
					...
					<td>
						<div class="form-group">
							<form:label path="emp.id" cssClass="col-sm-2 control-label">Professional:</form:label>
							<div class="col-xs-3">
								<form:input path="emp.fullName" cssClass="form-control input-sm" disabled="true"/>
			    			</div>
						</div>						
					</td>
				</tr>
						...
						<div class="form-group">
							...
							<c:choose>
								<c:when test="${isSuccess == true}">
									<input class="btn btn-default" type="button" value="Close" onclick="window.location.href = '/simsweb/goToSearchUserAccount';">
								</c:when>
								<c:otherwise>
									<input class="btn btn-default" type="submit" value="Submit/Update">
									<input class="btn btn-default" type="button" value="Cancel" onclick="window.location.href = '/simsweb/goToSearchUserAccount';">
								</c:otherwise>
							</c:choose>
							</div>
						</div>					
