...
<head>
	...
	<script>
		...
		function viewPOHeaderInRO(poId) {
			window.location.href = 'viewPOHeaderInRO?poId=' + poId;
		}
		
		function searchPOHeaderInRO(page,supplierName) {
			window.location.href = 'searchPOHeaderInRO?page=' + page + '&supplierName=' + supplierName;
		}
...
<body>
	...
	<form:form action="/simsweb/searchPOHeaderInRO" method="GET"  modelAttribute="poHeader" cssClass="form-horizontal" >
		...
		<div align="center" style="position: sticky; top: 0; background-color: white;">
			...
			<div align="center" style="padding-top: 10px;">
				...
				<label style="font-size: 24px;">Receive Order</label>		
			</div>
			<div align="center" style="padding-top: 10px;">
						...
						<div  class="table-responsive" style="width: 99%; padding-left: 10px;">
							<table class="table table-striped table-hover table-bordered table-responsive" style="font-size: 11px;">
								...
								<c:forEach items="${resultList}" var="model" varStatus = "row">
								    <tr>
								    	<td align="center"><a href="#" onclick="viewPOHeaderInRO(${model.id});" >Select</a></td>
			...
			<div style="width: 99%; padding-left: 10px;">
				<div style="position: relative; top: -15px; float: left; padding-left: 10px;">
					<ul class="pager">
						<c:if test="${currentPage != 1 && noOfPages > 0}">
							<li><a href="#" onclick="searchPOHeaderInRO(${currentPage - 1}, document.getElementById('supplierNameId').value);">Previous</a></li>
						</c:if>
						<c:if test="${currentPage lt noOfPages}">
							<li><a href="#" onclick="searchPOHeaderInRO(${currentPage + 1}, document.getElementById('supplierNameId').value);">Next</a></li>
						</c:if>
					</ul>
				</div>
				...
				<div style="position: relative; top: -15px; float: right;" >
					<ul class="pagination">
						...
						<c:if test="${currentPage != 1 && noOfPages > 0}">
							<li><a href="#" onclick="searchPOHeaderInRO(${currentPage - 1}, document.getElementById('supplierNameId').value);">&laquo;</a></li>
						</c:if>
						...
						<c:choose>
							<c:when test="${currentPage lt noOfPages && noOfPages > 10}">
								<c:forEach begin="${currentPage}" end="${currentPage+9}" var="i">
									<c:choose>
					                    ...
					                    <c:otherwise>	                
					                        <li><a href="#" onclick="searchPOHeaderInRO(${i}, document.getElementById('supplierNameId').value);">${i}</a></li>
					                    </c:otherwise>
					                </c:choose>
								</c:forEach>
							</c:when>
			           	...
			           	<c:if test="${currentPage lt noOfPages}">
			               	<li><a href="#" onclick="searchPOHeaderInRO(${currentPage + 1}, document.getElementById('supplierNameId').value);">&raquo;</a></li>
			            </c:if>
