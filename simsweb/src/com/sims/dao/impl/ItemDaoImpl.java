...
@Repository
public class ItemDaoImpl extends HibernateDaoUtil implements ItemDao{
	...
	public Item findByItemCode(String criteria) {
		...
		beginHibernateTransaction();
		try {
			list = session.createQuery("from Item where itemCode='" + criteria + "'").list();
			for (Item item: list) {
				model = item;
				break;
			}
...
//			query.where(builder.and(builder.like(root.get("patientNo"), "%" + criteria + "%"), builder.equal(root.get("active"), true)));
			...
			predicates.add(
					builder.and(
							builder.like(builder.upper(root.get("description")), "%" + searchCriteria.toUpperCase() + "%"),
							builder.equal(root.get("active"), true)));
//			predicates.add(builder.and(builder.equal(root.get("description"), "David"))); //can add more criteria
			query.where(builder.and(predicates.toArray(new Predicate[predicates.size()])));
			...
			q.setFirstResult(recordStart); //Index start at 0 (first record)
			q.setMaxResults(maxResult);
			List<Item> list2 = q.list();

			//Get totalRecordsCount for pagination
			CriteriaQuery<Long> queryCount = builder.createQuery(Long.class);
			queryCount.select(builder.count(queryCount.from(Item.class)));
			queryCount.where(builder.and(predicates.toArray(new Predicate[predicates.size()])));
			Query<Long> qCount = session.createQuery(queryCount);
			long totalNoOfRecords = qCount.getSingleResult();
	...
	public List<Item> getAllEntity() {
		...
		beginHibernateTransaction();
		try {
			list = session.createQuery("from Item order by description").list();
		 } catch (Exception e) {
	...
	public Item findById(int criteria) {
		...
		beginHibernateTransaction();
		try {
			model = session.get(Item.class, criteria);
