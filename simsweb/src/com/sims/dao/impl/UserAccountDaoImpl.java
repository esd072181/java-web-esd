...
import com.sims.util.EncryptUtil;
...
public class UserAccountDaoImpl extends HibernateDaoUtil implements UserAccountDao{
	...
	public Map<Object, Object> findBy(Map<Object, Object> mapCriteria) {
		...
		try {
			...
			predicates.add(
					builder.and(
							builder.or(builder.like(builder.upper(root.get("emp").get("lastName")), "%" + searchCriteria.toUpperCase() + "%"), builder.like(builder.upper(root.get("emp").get("firstName")), "%" + searchCriteria.toUpperCase() + "%")),
							builder.equal(root.get("active"), true)));
			query.where(builder.and(predicates.toArray(new Predicate[predicates.size()])));
			query.orderBy(builder.asc(root.get("emp").get("lastName")), builder.asc(root.get("emp").get("firstName")), builder.asc(root.get("emp").get("middleName")));
