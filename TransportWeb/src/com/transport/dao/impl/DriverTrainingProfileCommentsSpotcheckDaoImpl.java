 ...
 * @since 16Sep2019
 */
public class DriverTrainingProfileCommentsSpotcheckDaoImpl implements DriverTrainingProfileCommentsDao {
	
	private final static Logger logger = Logger.getLogger(DriverTrainingProfileCommentsSpotcheckDaoImpl.class);
	...
	public Map<String, Object> save(HashMap<String, Object> dataMap) throws Exception{
		...
		StringBuffer qry =  new StringBuffer("insert into transport.tran_driver_training_profile_comments_spotcheck (");
		...
		StringBuffer qryLog =  new StringBuffer("insert into transport.tran_driver_training_profile_comments_spotcheck (");
		...
		TransportUtils.writeLogDebug(logger, "SQL: "+qryLog.toString());
		  ...
		  try {
			  ...
			  if (statusInt == 1) {
				  conn.commit();
				  System.out.println("Inserted into DriverTrainingProfileCommentSpotcheck  table successfully..");
				  status = true;
			  }
		  } catch (Exception e) {
			  conn.rollback();
	...
	public Map<String, Object> update(HashMap<String, Object> dataMap) throws Exception{
		...
		StringBuffer qry =  new StringBuffer("update transport.tran_driver_training_profile_comments_spotcheck set ");	
			qry.append(" drivertrainingprofileid=? ");
			qry.append(" ,remarks=? ");
		...
		StringBuffer qryLog =  new StringBuffer("update transport.tran_driver_training_profile_comments_spotcheck set ");	
		...
		TransportUtils.writeLogDebug(logger, "SQL: "+qryLog.toString());
		 ...
		 try {
			...
			if (statusInt == 1) {
				conn.commit();
				System.out.println("DriverTrainingProfileCommentSpotcheck  record (id: " +model.getId()+") updated successfully..");
				status = true;
			}
		} catch (Exception e) {
			conn.rollback();
	...
	public Map<String, Object> delete(HashMap<String, Object> dataMap) throws Exception{
		...
		TransportUtils.writeLogDebug(logger, "SQL: "+qryLog.toString());
		 ...
		 try {
			...
			if (statusInt == 1) {
				conn.commit();
				System.out.println("DriverTrainingProfileCommentSpotcheck  record (id: " +model.getId()+") deleted successfully..");
				status = true;
			}
		} catch (Exception e) {
			conn.rollback();
		...
		TransportUtils.writeLogInfo(logger, MiscConstant.LOGGING_MESSSAGE_SEARCH);
			 ...
			 try {
				 ...
				 StringBuffer sql = new StringBuffer("select a.id,a.drivertrainingprofileid,a.remarks, ");
					 	...
					 	sql.append(" from transport.tran_driver_training_profile_comments_spotcheck a, transport.file_user b,  ");
		     ...
		     try {
			    ...
			    sqlCount = new StringBuffer("select count(*) from transport.tran_driver_training_profile_comments_spotcheck a where a.drivertrainingprofileid = ? and a.active = true");	 
		    ...
		    DriverTrainingProfileComment  model = (DriverTrainingProfileComment ) criteriaMap.get(MapConstant.CLASS_DATA);
			 ...
			 try {
				 ...
				 StringBuffer sql = new StringBuffer();				
				 	...
				 	sql.append(" from transport.tran_driver_training_profile_comments_spotcheck a ");
