...
public class CorrectionsModule implements TransportModule {
	
	private final static Logger logger = Logger.getLogger(CorrectionsModule.class);
	...
	public Map<String, Object> executeRequest(HashMap<String, Object> dataMap)
		...
		try {
			...
			CorrectionsBo bo = (CorrectionsBo) dataMap.get(MapConstant.BEAN);
			...
			switch(action) {
				...
				case ActionConstant.GET_ACTIVE_DATA: 
						...
						if (subAction!=0 && subAction == SubActionConstant.GET_NO_CLOSURE_DATA) {
							returnMap = bo.getNoClosureData();	
						} else if (subAction!=0 && subAction == SubActionConstant.GET_CORRECTIONS_BY_ROOT_CAUSE) {
							returnMap = bo.getCorrectionsByRootCause(dataMap);
						} else {
