...
import com.transport.dao.TerminalDao;
...
public class TerminalDaoImpl implements TerminalDao {
	...
	private final static Logger logger = Logger.getLogger(TerminalDaoImpl.class);
	...
	public Map<String, Object> save(HashMap<String, Object> dataMap) throws Exception{
		...
		Terminal model = (Terminal) dataMap.get(MapConstant.CLASS_DATA);
		...
		StringBuffer qry =  new StringBuffer("insert into transport.file_terminal (");
		  		qry.append("name ");
		...
		StringBuffer qryLog =  new StringBuffer("insert into transport.file_terminal (");
				qryLog.append("name ");
		...
		TransportUtils.writeLogDebug(logger, "SQL: "+qryLog.toString());
		  ...
		  try {
			  ...
			  if (statusInt == 1) {
				  conn.commit();
				  System.out.println("Inserted into Terminal table successfully..");
				  status = true;
			  }
		  } catch (Exception e) {
			  conn.rollback();
	...
	public Map<String, Object> update(HashMap<String, Object> dataMap) throws Exception{
		...
		StringBuffer qry =  new StringBuffer("update transport.file_terminal set ");	
			qry.append(" name=? ");
		...
		StringBuffer qryLog =  new StringBuffer("update transport.file_terminal set ");	
		...
		TransportUtils.writeLogDebug(logger, "SQL: "+qryLog.toString());
		 ...
		 try {
			...
			if (statusInt == 1) {
				conn.commit();
				System.out.println("Terminal record (id: " +model.getId()+") updated successfully..");
				status = true;
			}
		} catch (Exception e) {
			conn.rollback();
	...
	public Map<String, Object> delete(HashMap<String, Object> dataMap) throws Exception{
		...
		TransportUtils.writeLogDebug(logger, "SQL: "+qryLog.toString());
		 ...
		 try {
			...
			if (statusInt == 1) {
				conn.commit();
				System.out.println("Terminal record (id: " +model.getId()+") deleted successfully..");
				status = true;
			}
		} catch (Exception e) {
			conn.rollback();
	...
	public Map<String, Object> search(HashMap<String, Object> criteriaMap) throws Exception{
		...
		TransportUtils.writeLogInfo(logger, MiscConstant.LOGGING_MESSSAGE_SEARCH);
			 ...
			 PreparedStatement pstmt = null;
			  ...
			  List<Terminal> rsList = new ArrayList<Terminal>();
			 ...
			 try {
				 ...
				 if (category.equals(ActionConstant.SEARCHALL)) {
					 sql = new StringBuffer("select id,name,createdby,createdon,modifiedby,modifiedon,version,active ");
					 	sql.append(" from transport.file_terminal ");
					 ...
					 sql = new StringBuffer("select id,name,createdby,createdon,modifiedby,modifiedon,version,active ");
					 	sql.append(" from transport.file_terminal ");
					 	sql.append(" where ");
					 	sql.append(" (name ilike '%"+criteria+"%')" );
				...
				if (category.equals(ActionConstant.SEARCHALL)) {
							 sqlLog = new StringBuffer("select id,name,createdby,createdon,modifiedby,modifiedon,version,active ");
							 sqlLog.append(" from transport.file_terminal ");
							 ...
							 sqlLog.append(" from transport.file_terminal ");
							 sqlLog.append(" where ");
							 sqlLog.append(" (name ilike '%"+criteria+"%')" );
		     ...
		     try {
			     ...
			     if (category.equals(ActionConstant.SEARCHALL)) {
			    	 sqlCount = new StringBuffer("select count(*) from transport.file_terminal where active = true");	 
			     }else {
			    	 sqlCount = new StringBuffer("select count(*) from transport.file_terminal where  (name ilike '%"+criteria+"%') and active = true");	 
			     } 

				StringBuffer sqlCountLog = null;
				if (category.equals(ActionConstant.SEARCHALL)) {
					 sqlCountLog = new StringBuffer("select count(*) from transport.file_terminal where active = true");	 
				}else {
					 sqlCountLog = new StringBuffer("select count(*) from transport.file_terminal where  (name ilike '%"+criteria+"%') and active = true");	 
				} 
	...
	public Map<String, Object> getDataById(HashMap<String, Object> criteriaMap)
		...
		TransportUtils.writeLogInfo(logger, MiscConstant.LOGGING_MESSSAGE_GETDATA_BY_ID);
		    ...
		    Terminal model = (Terminal) criteriaMap.get(MapConstant.CLASS_DATA);
			 ...
			 try {
				 ...
				 StringBuffer sqlLog = new StringBuffer();				
				 	sqlLog.append("select id,name,createdby,createdon,modifiedby,modifiedon,version,active ");
				 	sqlLog.append("from transport.file_terminal ");
		...
		TransportUtils.writeLogInfo(logger, MiscConstant.LOGGING_MESSSAGE_GET_INACTIVE_DATA);
		 ...
		 int limit = (int) criteriaMap.get(MapConstant.PAGINATION_LIMIT);
		     ...
		     try {
			    ...
			    sqlCount = new StringBuffer("select count(*) from transport.file_terminal where active = false");	 
				
			    StringBuffer sqlCountLog = new StringBuffer("select count(*) from transport.file_terminal where active = false");	 
	...
	public Map<String, Object> restore(HashMap<String, Object> dataMap) throws Exception{
		...
		TransportUtils.writeLogDebug(logger, "SQL: "+qryLog.toString());
		 ...
		 try {
			...
			if (statusInt == 1) {
				conn.commit();
				System.out.println("Terminal record (id: " +model.getId()+") restored successfully..");
				status = true;
			}
		} catch (Exception e) {
			conn.rollback();
