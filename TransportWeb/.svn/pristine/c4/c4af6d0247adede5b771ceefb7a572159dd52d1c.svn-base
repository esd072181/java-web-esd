...
public class VerificationAndValidationModule implements TransportModule {
	...
	public Map<String, Object> executeRequest(HashMap<String, Object> dataMap)
		...
		try {
			...
			VerificationAndValidationBo bo = (VerificationAndValidationBo) dataMap.get(MapConstant.BEAN);
			...
			switch(action) {
				...
				case ActionConstant.GET_ACTIVE_DATA:
						...
						if (subAction!=0 && subAction == SubActionConstant.GET_REMINDERS) {
							returnMap = bo.getVerificationRemiders(dataMap);	
						} else {
