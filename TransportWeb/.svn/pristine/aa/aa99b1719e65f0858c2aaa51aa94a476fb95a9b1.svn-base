...
import com.transport.dao.RootCauseDao;
...
public class RootCauseDaoImpl implements RootCauseDao {
	...
	private final static Logger logger = Logger.getLogger(RootCauseDaoImpl.class);
	...
	public Map<String, Object> save(HashMap<String, Object> dataMap) throws Exception{
		...
		RootCause model = (RootCause) dataMap.get(MapConstant.CLASS_DATA);
		...
		StringBuffer qry =  new StringBuffer("insert into transport.file_rootcause (");
		...
		StringBuffer qryLog =  new StringBuffer("insert into transport.file_rootcause (");
		...
		TransportUtils.writeLogDebug(logger, "SQL: "+qryLog.toString());
		  ...
		  try {
			  ...
			  if (statusInt == 1) {
				  conn.commit();
				  System.out.println("Inserted into Root Cause table successfully..");
				  status = true;
			  }
		  } catch (Exception e) {
			  conn.rollback();
	...
	public Map<String, Object> update(HashMap<String, Object> dataMap) throws Exception{
		...
		StringBuffer qry =  new StringBuffer("update transport.file_rootcause set ");	
			qry.append(" description=? ");
		...
		StringBuffer qryLog =  new StringBuffer("update transport.file_rootcause set ");	
		...
		TransportUtils.writeLogDebug(logger, "SQL: "+qryLog.toString());
		 ...
		 try {
			...
			if (statusInt == 1) {
				conn.commit();
				System.out.println("Root Cause record (id: " +model.getId()+") updated successfully..");
				status = true;
			}
		} catch (Exception e) {
			conn.rollback();
	...
	public Map<String, Object> delete(HashMap<String, Object> dataMap) throws Exception{
		...
		TransportUtils.writeLogDebug(logger, "SQL: "+qryLog.toString());
		 ...
		 try {
			...
			if (statusInt == 1) {
				conn.commit();
				System.out.println("Root Cause record (id: " +model.getId()+") deleted successfully..");
				status = true;
			}
		} catch (Exception e) {
			conn.rollback();
	...
	public Map<String, Object> search(HashMap<String, Object> criteriaMap) throws Exception{
		...
		TransportUtils.writeLogInfo(logger, MiscConstant.LOGGING_MESSSAGE_SEARCH);
			 ...
			 PreparedStatement pstmt = null;
			  ...
			  List<RootCause> rsList = new ArrayList<RootCause>();
			 ...
			 try {
				 ...
				 if (category.equals(ActionConstant.SEARCHALL)) {
					 sql = new StringBuffer("select id,description,createdby,createdon,modifiedby,modifiedon,version,active ");
					 	sql.append(" from transport.file_rootcause ");
				...
				if (category.equals(ActionConstant.SEARCHALL)) {
							 ...
							 sqlLog.append(" from transport.file_rootcause ");
		     ...
		     try {
			     ...
			     if (category.equals(ActionConstant.SEARCHALL)) {
			    	 sqlCount = new StringBuffer("select count(*) from transport.file_rootcause where active = true");	 
			     }else {
			    	 sqlCount = new StringBuffer("select count(*) from transport.file_rootcause where  (description ilike '%"+criteria+"%') and active = true");	 
			     } 

				StringBuffer sqlCountLog = null;
				if (category.equals(ActionConstant.SEARCHALL)) {
					 sqlCountLog = new StringBuffer("select count(*) from transport.file_rootcause where active = true");	 
				}else {
					 sqlCountLog = new StringBuffer("select count(*) from transport.file_rootcause where  (description ilike '%"+criteria+"%') and active = true");	 
				} 
	...
	public Map<String, Object> getDataById(HashMap<String, Object> criteriaMap)
		...
		TransportUtils.writeLogInfo(logger, MiscConstant.LOGGING_MESSSAGE_GETDATA_BY_ID);
		    ...
		    RootCause model = (RootCause) criteriaMap.get(MapConstant.CLASS_DATA);
		...
		TransportUtils.writeLogInfo(logger, MiscConstant.LOGGING_MESSSAGE_GET_INACTIVE_DATA);
		 ...
		 int limit = (int) criteriaMap.get(MapConstant.PAGINATION_LIMIT);
		     ...
		     try {
			    ...
			    sqlCount = new StringBuffer("select count(*) from transport.file_rootcause where active = false");	 

				StringBuffer sqlCountLog = null;
				sqlCountLog = new StringBuffer("select count(*) from transport.file_rootcause where active = false");	 
	...
	public Map<String, Object> restore(HashMap<String, Object> dataMap) throws Exception{
		...
		TransportUtils.writeLogDebug(logger, "SQL: "+qryLog.toString());
		 ...
		 try {
			...
			if (statusInt == 1) {
				conn.commit();
				System.out.println("Root Cause record (id: " +model.getId()+") restored successfully..");
				status = true;
			}
		} catch (Exception e) {
			conn.rollback();
