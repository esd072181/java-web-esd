...
import com.pibs.form.MedicationStatementFormBean;
...
public class MedicationStatementAction extends Action {
	
	private final static Logger logger = Logger.getLogger(MedicationStatementAction.class);
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		MedicationStatementFormBean formBean = (MedicationStatementFormBean) form;
		...
		if (request.getSession().getAttribute(MiscConstant.USER_SESSION) == null) {
			...
			if (command!=null) {
				...
				int module = ModuleConstant.MEDICATION_STATEMENT;
				...
				if (command.equalsIgnoreCase(ParamConstant.AJAX_GO_TO_CHILD)) {		
			        ...
			        if (resultMap!=null && !resultMap.isEmpty()) {
				        model =  (MedicationStatement) resultMap.get(MapConstant.CLASS_DATA);		        		
					...
					//populateModel
					MedicationStatement model = (MedicationStatement) formBean.populateModel(formBean);
