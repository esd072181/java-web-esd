...
public class BillingDiscountDaoImpl implements BillingDiscountDao {
	
	private final static Logger logger = Logger.getLogger(BillingDiscountDaoImpl.class);
	...
	public Map<String, Object> save(HashMap<String, Object> dataMap) throws Exception{
		...
		BillingDiscount model = (BillingDiscount) dataMap.get(MapConstant.CLASS_DATA);
		...
		StringBuffer qry =  new StringBuffer("insert into pibs.tran_patient_discount (");  		
				qry.append("patientcasesystemid ");
				qry.append(",discountid ");
		  		qry.append(",amount ");
		...
		PIBSUtils.writeLogDebug(logger, "SQL: "+qry.toString());
		  ...
		  try {
			  ...
			  pstmt.setInt(2, model.getDiscountId());
			  ...
			  if (statusInt == 1) {
				  conn.commit();
				  System.out.println("Inserted into Billing Discount table successfully..");
				  status = true;
			  }
		  } catch (Exception e) {
			  conn.rollback();
	...
	public Map<String, Object> update(HashMap<String, Object> dataMap) throws Exception{
		...
		StringBuffer qry =  new StringBuffer("update pibs.tran_patient_discount set ");	
			qry.append(" discountid=? ");
			qry.append(" ,amount=? ");
		...
		PIBSUtils.writeLogDebug(logger, "SQL: "+qry.toString());
		 ...
		 try {
			...
			pstmt.setInt(1, model.getDiscountId());
			...
			if (statusInt == 1) {
				conn.commit();
				System.out.println("Billing Discount record (id: " +model.getId()+") updated successfully..");
				status = true;
			}
		} catch (Exception e) {
			conn.rollback();
	...
	public Map<String, Object> search(HashMap<String, Object> criteriaMap) throws Exception{
		...
		PIBSUtils.writeLogInfo(logger, MiscConstant.LOGGING_MESSSAGE_SEARCH);
		 	...
		 	BillingDiscount modelCriteria = (BillingDiscount) criteriaMap.get(MapConstant.CLASS_DATA);
			 ...
			 List<BillingDiscount> rsList = new ArrayList<BillingDiscount>();
			 ...
			 try {
				...
				PIBSUtils.writeLogDebug(logger, "SQL: "+sql.toString());
				 ...
				 while(rs.next()) {
		    		 ...
		    		 model.setDiscountId(rs.getInt(3));
		    		 model.setAmount(rs.getBigDecimal(4));
		    		 model.setDiscountDescription(rs.getString(5));
		    		 rsList.add(model);
				 }				 
	...
	public Map<String, Object> getDataById(HashMap<String, Object> criteriaMap)
		...
		PIBSUtils.writeLogInfo(logger, MiscConstant.LOGGING_MESSSAGE_GETDATA_BY_ID);
		    ...
		    BillingDiscount model = (BillingDiscount) criteriaMap.get(MapConstant.CLASS_DATA);
	...
	public Map<String, Object> delete(HashMap<String, Object> dataMap) throws Exception{
		...
		PIBSUtils.writeLogDebug(logger, "SQL: "+qry.toString());
		 ...
		 try {
			...
			if (statusInt == 1) {
				conn.commit();
				System.out.println("Billing Discount record (id: " +model.getId()+") deleted successfully..");
				status = true;
			}
		} catch (Exception e) {
			conn.rollback();
