...
public class LaboratoryExaminationDaoImpl implements LaboratoryExaminationDao {
	
	private final static Logger logger = Logger.getLogger(LaboratoryExaminationDaoImpl.class);
	...
	public Map<String, Object> save(HashMap<String, Object> dataMap) throws Exception{
		...
		LaboratoryExamination model = (LaboratoryExamination) dataMap.get(MapConstant.CLASS_DATA);
		...
		StringBuffer qry =  new StringBuffer("insert into pibs.file_laboratory_examination (");
		...
		StringBuffer qryLog =  new StringBuffer("insert into pibs.file_laboratory_examination (");
		...
		PIBSUtils.writeLogDebug(logger, "SQL: "+qryLog.toString());
		  ...
		  try {
			  ...
			  if (statusInt == 1) {
				  conn.commit();
				  System.out.println("Inserted into Laboratory Examination table successfully..");
				  status = true;
			  }
		  } catch (Exception e) {
			  conn.rollback();
	...
	public Map<String, Object> update(HashMap<String, Object> dataMap) throws Exception{
		...
		StringBuffer qry =  new StringBuffer("update pibs.file_laboratory_examination set ");	
			qry.append(" description=? ");
			qry.append(" ,remarks=? ");
			qry.append(" ,fee=? ");
		...
		StringBuffer qryLog =  new StringBuffer("update pibs.file_laboratory_examination set ");	
		...
		PIBSUtils.writeLogDebug(logger, "SQL: "+qryLog.toString());
		 ...
		 try {
			...
			if (statusInt == 1) {
				conn.commit();
				System.out.println("Laboratory Examination record (id: " +model.getId()+") updated successfully..");
				status = true;
			}
		} catch (Exception e) {
			conn.rollback();
	...
	public Map<String, Object> delete(HashMap<String, Object> dataMap) throws Exception{
		...
		PIBSUtils.writeLogDebug(logger, "SQL: "+qryLog.toString());
		 ...
		 try {
			...
			if (statusInt == 1) {
				conn.commit();
				System.out.println("Laboratory Examination record (id: " +model.getId()+") deleted successfully..");
				status = true;
			}
		} catch (Exception e) {
			conn.rollback();
	...
	public Map<String, Object> search(HashMap<String, Object> criteriaMap) throws Exception{
		...
		PIBSUtils.writeLogInfo(logger, MiscConstant.LOGGING_MESSSAGE_SEARCH);
			 ...
			 PreparedStatement pstmt = null;
			  ...
			  List<LaboratoryExamination> rsList = new ArrayList<LaboratoryExamination>();
			 ...
			 try {
				 ...
				 if (category.equals(ActionConstant.SEARCHALL)) {
					 sql = new StringBuffer("select id,description,remarks,fee,createdby,createdon,modifiedby,modifiedon,version,active ");
					 	sql.append(" from pibs.file_laboratory_examination ");
		     ...
		     try {
			     ...
			     if (category.equals(ActionConstant.SEARCHALL)) {
			    	 sqlCount = new StringBuffer("select count(*) from pibs.file_laboratory_examination where active = true");	 
			     }else {
			    	 sqlCount = new StringBuffer("select count(*) from pibs.file_laboratory_examination where  (description ilike '%"+criteria+"%' or remarks ilike '%"+criteria+"%') and active = true");	 
			     } 
	...
	public Map<String, Object> getDataById(HashMap<String, Object> criteriaMap)
		...
		PIBSUtils.writeLogInfo(logger, MiscConstant.LOGGING_MESSSAGE_GETDATA_BY_ID);
		    ...
		    LaboratoryExamination model = (LaboratoryExamination) criteriaMap.get(MapConstant.CLASS_DATA);
			 ...
			 try {
				 ...
				 StringBuffer sql = new StringBuffer();				
				 	sql.append("select id,description,remarks,fee,createdby,createdon,modifiedby,modifiedon,version,active ");
				 	sql.append("from pibs.file_laboratory_examination ");
				 	sql.append("where id = ?");

				 StringBuffer sqlLog = new StringBuffer();				
				 	sqlLog.append("select id,description,remarks,fee,createdby,createdon,modifiedby,modifiedon,version,active ");
				 	sqlLog.append("from pibs.file_laboratory_examination ");
	...
	public Map<String, Object> getInActiveData(HashMap<String, Object> criteriaMap) throws Exception {
		...
		PIBSUtils.writeLogInfo(logger, MiscConstant.LOGGING_MESSSAGE_GET_INACTIVE_DATA);
		 ...
		 int limit = (int) criteriaMap.get(MapConstant.PAGINATION_LIMIT);
			 ...
			 PreparedStatement pstmt = null;
		     Map<String, Object> returnMap = null;
			 List<LaboratoryExamination> rsList = new ArrayList<>();
		     ...
		     try {
			    ...
			    sqlCount = new StringBuffer("select count(*) from pibs.file_laboratory_examination where active = false");	 
	...
	public Map<String, Object> searchForListReport(HashMap<String, Object> criteriaMap) throws Exception {
		...
		PIBSUtils.writeLogInfo(logger, MiscConstant.LOGGING_MESSSAGE_SEARCH);
			 ...
			 try {
				 ...
				 sql = new StringBuffer("select a.id,a.description,a.remarks,a.fee,a.createdby,a.createdon,a.modifiedby,a.modifiedon,a.version,a.active ");
				 	sql.append(" from pibs.file_laboratory_examination a  ");
				 	sql.append(" where a.active = true ");
			    	if (MiscConstant.LOV_LABORATORY_EXAMINATION_REPORT_SEARCH_CRITERIA_DESCRIPTION == searchCriteria) {
			    		sql.append(" and a.description ilike '%"+searchValue+"%' ");
			    	}
			    	if (MiscConstant.LOV_LABORATORY_EXAMINATION_REPORT_SEARCH_CRITERIA_REMARKS == searchCriteria) {
