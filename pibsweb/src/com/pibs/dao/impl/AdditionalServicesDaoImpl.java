	...
	public Map<String, Object> getInActiveData(HashMap<String, Object> criteriaMap) throws Exception {
		...
		PIBSUtils.writeLogInfo(logger, MiscConstant.LOGGING_MESSSAGE_GET_INACTIVE_DATA);
		 ...
		 int limit = (int) criteriaMap.get(MapConstant.PAGINATION_LIMIT);
			 ...
			 PreparedStatement pstmt = null;
		     Map<String, Object> returnMap = null;
			 List<AdditionalServices> rsList = new ArrayList<>();
		     ...
		     try {
			    ...
			    sqlCount = new StringBuffer("select count(*) from pibs.file_additional_services where active = false");	 
	...
	public Map<String, Object> searchForListReport(HashMap<String, Object> criteriaMap) throws Exception {
		...
		PIBSUtils.writeLogInfo(logger, MiscConstant.LOGGING_MESSSAGE_SEARCH);
			 ...
			 try {
				 ...
				 sql = new StringBuffer("select a.id,a.description,a.remarks,a.fee,b.description as category,a.createdby,a.createdon,a.modifiedby,a.modifiedon,a.version,a.active ");
				 	sql.append(" from pibs.file_additional_services a, pibs.file_additional_services_category b ");
				 	sql.append(" where a.additionalservicescategoryid = b.id ");
				 	sql.append(" and a.active = true ");
			    	if (MiscConstant.LOV_ADDITIONAL_SERVICES_REPORT_SEARCH_CRITERIA_CATEGORY == searchCriteria) {
			    		sql.append(" and b.description ilike '%"+searchValue+"%' ");
			    	}
			    	if (MiscConstant.LOV_ADDITIONAL_SERVICES_REPORT_SEARCH_CRITERIA_DESCRIPTION == searchCriteria) {
			    		sql.append(" and a.description ilike '%"+searchValue+"%' ");
			    	}
			    	if (MiscConstant.LOV_ADDITIONAL_SERVICES_REPORT_SEARCH_CRITERIA_REMARKS == searchCriteria) {
