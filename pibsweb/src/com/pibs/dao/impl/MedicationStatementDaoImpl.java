...
public class MedicationStatementDaoImpl implements MedicationStatementDao {
	...
	private final static Logger logger = Logger.getLogger(MedicationStatementDaoImpl.class);
	...
	public Map<String, Object> save(HashMap<String, Object> dataMap) throws Exception{
		...
		MedicationStatement model = (MedicationStatement) dataMap.get(MapConstant.CLASS_DATA);
		...
		StringBuffer qry =  new StringBuffer("insert into pibs.tran_patient_medication_statement (");  		
				qry.append("patientcasesystemid ");
				qry.append(",medicationstatement ");
		  		qry.append(",remarks ");
		  		qry.append(",dateofstatement ");
		...
		PIBSUtils.writeLogDebug(logger, "SQL: "+qry.toString());
		  ...
		  try {
			  ...
			  pstmt.setString(2, model.getMedicationStatement());
			  pstmt.setString(3, model.getRemarks());
			  pstmt.setDate(4, model.getDateOfStatement());
			  ...
			  if (statusInt == 1) {
				  conn.commit();
				  System.out.println("Inserted into Medication Statement table successfully..");
				  status = true;
			  }
		  } catch (Exception e) {
			  conn.rollback();
	...
	public Map<String, Object> update(HashMap<String, Object> dataMap) throws Exception{
		...
		StringBuffer qry =  new StringBuffer("update pibs.tran_patient_medication_statement set ");	
			qry.append(" medicationstatement=? ");
			qry.append(" ,remarks=? ");
			qry.append(" ,dateofstatement=? ");
		...
		PIBSUtils.writeLogDebug(logger, "SQL: "+qry.toString());
		 ...
		 try {
			...
			pstmt.setString(1, model.getMedicationStatement());
			pstmt.setString(2, model.getRemarks());
			pstmt.setDate(3, model.getDateOfStatement());
			...
			if (statusInt == 1) {
				conn.commit();
				System.out.println("Diagnosis record (id: " +model.getId()+") updated successfully..");
				status = true;
			}
		} catch (Exception e) {
			conn.rollback();
	...
	public Map<String, Object> getDataById(HashMap<String, Object> criteriaMap)
		...
		PIBSUtils.writeLogInfo(logger, MiscConstant.LOGGING_MESSSAGE_GETDATA_BY_ID);
		    ...
		    MedicationStatement model = (MedicationStatement) criteriaMap.get(MapConstant.CLASS_DATA);
			 ...
			 try {
				 ...
				 StringBuffer sql = new StringBuffer();				
				 	sql.append("select id,medicationstatement,remarks,dateofstatement,createdby,createdon,modifiedby,modifiedon,version,active ");
				 	sql.append("from pibs.tran_patient_medication_statement ");
				 ...
				 while(rs.next()) {
		    		 ...
		    		 model.setMedicationStatement(rs.getString(2));
		    		 model.setRemarks(rs.getString(3));
		    		 model.setDateOfStatement(rs.getDate(4));
				 }				 
