...
import com.pibs.dao.SpecializationDao;
...
public class SpecializationDaoImpl implements SpecializationDao {
	...
	private final static Logger logger = Logger.getLogger(SpecializationDaoImpl.class);
	...
	public Map<String, Object> save(HashMap<String, Object> dataMap) throws Exception{
		...
		Specialization model = (Specialization) dataMap.get(MapConstant.CLASS_DATA);
		...
		StringBuffer qry =  new StringBuffer("insert into pibs.file_specialization (");
		...
		StringBuffer qryLog =  new StringBuffer("insert into pibs.file_specialization (");
		...
		PIBSUtils.writeLogDebug(logger, "SQL: "+qryLog.toString());
		  ...
		  try {
			  ...
			  if (statusInt == 1) {
				  conn.commit();
				  System.out.println("Inserted into Specialization table successfully..");
				  status = true;
			  }
		  } catch (Exception e) {
			  conn.rollback();
	...
	public Map<String, Object> update(HashMap<String, Object> dataMap) throws Exception{
		...
		StringBuffer qry =  new StringBuffer("update pibs.file_specialization set ");	
			qry.append(" entityName=? ");
			qry.append(" ,description=? ");
		...
		StringBuffer qryLog =  new StringBuffer("update pibs.file_specialization set ");	
		...
		PIBSUtils.writeLogDebug(logger, "SQL: "+qryLog.toString());
		 ...
		 try {
			...
			if (statusInt == 1) {
				conn.commit();
				System.out.println("Specialization record (id: " +model.getId()+") updated successfully..");
				status = true;
			}
		} catch (Exception e) {
			conn.rollback();
	...
	public Map<String, Object> delete(HashMap<String, Object> dataMap) throws Exception{
		...
		PIBSUtils.writeLogDebug(logger, "SQL: "+qryLog.toString());
		 ...
		 try {
			...
			if (statusInt == 1) {
				conn.commit();
				System.out.println("Specialization record (id: " +model.getId()+") deleted successfully..");
				status = true;
			}
		} catch (Exception e) {
			conn.rollback();
	...
	public Map<String, Object> search(HashMap<String, Object> criteriaMap) throws Exception{
		...
		PIBSUtils.writeLogInfo(logger, MiscConstant.LOGGING_MESSSAGE_SEARCH);
			 ...
			 PreparedStatement pstmt = null;
			  ...
			  List<Specialization> rsList = new ArrayList<Specialization>();
			 ...
			 try {
				 ...
				 if (category.equals(ActionConstant.SEARCHALL)) {
					 sql = new StringBuffer("select id,entityName,description,createdby,createdon,modifiedby,modifiedon,version,active ");
					 	sql.append(" from pibs.file_specialization ");
		     ...
		     try {
			     ...
			     if (category.equals(ActionConstant.SEARCHALL)) {
			    	 sqlCount = new StringBuffer("select count(*) from pibs.file_specialization where active = true");	 
			     }else {
			    	 sqlCount = new StringBuffer("select count(*) from pibs.file_specialization where  (entityName ilike '%"+criteria+"%' or description ilike '%"+criteria+"%') and active = true");	 
			     } 
	...
	public Map<String, Object> getDataById(HashMap<String, Object> criteriaMap)
		...
		PIBSUtils.writeLogInfo(logger, MiscConstant.LOGGING_MESSSAGE_GETDATA_BY_ID);
		    ...
		    Specialization model = (Specialization) criteriaMap.get(MapConstant.CLASS_DATA);
			 ...
			 try {
				 ...
				 StringBuffer sql = new StringBuffer();				
				 	sql.append("select id,entityName,description,createdby,createdon,modifiedby,modifiedon,version,active ");
				 	sql.append("from pibs.file_specialization ");
				 	sql.append("where id = ?");

				 StringBuffer sqlLog = new StringBuffer();				
				 	sqlLog.append("select id,entityName,description,createdby,createdon,modifiedby,modifiedon,version,active ");
				 	sqlLog.append("from pibs.file_specialization ");
	...
	public Map<String, Object> getInActiveData(HashMap<String, Object> criteriaMap) throws Exception {
		...
		PIBSUtils.writeLogInfo(logger, MiscConstant.LOGGING_MESSSAGE_GET_INACTIVE_DATA);
		 ...
		 int limit = (int) criteriaMap.get(MapConstant.PAGINATION_LIMIT);
			 ...
			 PreparedStatement pstmt = null;
		     Map<String, Object> returnMap = null;
			 List<Specialization> rsList = new ArrayList<>();
		     ...
		     try {
			    ...
			    sqlCount = new StringBuffer("select count(*) from pibs.file_specialization where active = false");	 
