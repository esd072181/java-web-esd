...
public class ProfessionalTypeDaoImpl implements ProfessionalTypeDao {
	...
	public Map<String, Object> save(HashMap<String, Object> dataMap) throws Exception{
		...
		ProfessionalType model = (ProfessionalType) dataMap.get(MapConstant.CLASS_DATA);
		...
		StringBuffer qry =  new StringBuffer("insert into pibs.file_professional_type (");  		
		...
		StringBuffer qryLog =  new StringBuffer("insert into pibs.file_professional_type (");
		...
		PIBSUtils.writeLogDebug(logger, "SQL: "+qryLog.toString());
		  ...
		  try {
			  ...
			  if (statusInt == 1) {
				  conn.commit();
				  System.out.println("Inserted into Professional Type table successfully..");
				  status = true;
			  }
		  } catch (Exception e) {
			  conn.rollback();
	...
	public Map<String, Object> update(HashMap<String, Object> dataMap) throws Exception{
		...
		StringBuffer qry =  new StringBuffer("update pibs.file_professional_type set ");	
			qry.append(" description=? ");
			qry.append(" ,remarks=? ");
		...
		StringBuffer qryLog =  new StringBuffer("update pibs.file_professional_type set ");		
		...
		PIBSUtils.writeLogDebug(logger, "SQL: "+qryLog.toString());
		 ...
		 try {
			...
			if (statusInt == 1) {
				conn.commit();
				System.out.println("Professional Type record (id: " +model.getId()+") updated successfully..");
				status = true;
			}
		} catch (Exception e) {
			conn.rollback();
	...
	public Map<String, Object> delete(HashMap<String, Object> dataMap) throws Exception{
		...
		PIBSUtils.writeLogDebug(logger, "SQL: "+qryLog.toString());
		 ...
		 try {
			...
			if (statusInt == 1) {
				conn.commit();
				System.out.println("Professional Type record (id: " +model.getId()+") deleted successfully..");
				status = true;
			}
		} catch (Exception e) {
			conn.rollback();
	...
	public Map<String, Object> search(HashMap<String, Object> criteriaMap) throws Exception{
		...
		PIBSUtils.writeLogInfo(logger, MiscConstant.LOGGING_MESSSAGE_SEARCH);
			 ...
			 PreparedStatement pstmt = null;
			  ...
			  List<ProfessionalType> rsList = new ArrayList<ProfessionalType>();
			 ...
			 try {
				 ...
				 if (category.equals(ActionConstant.SEARCHALL)) {
					 sql = new StringBuffer("select id,description,remarks,createdby,createdon,modifiedby,modifiedon,version,active ");
					 	sql.append(" from pibs.file_professional_type ");
		     ...
		     try {
			     ...
			     if (category.equals(ActionConstant.SEARCHALL)) {
			    	 sqlCount = new StringBuffer("select count(*) from pibs.file_professional_type where active = true");	 
			     }else {
			    	 sqlCount = new StringBuffer("select count(*) from pibs.file_professional_type where  (description ilike '%"+criteria+"%' or remarks ilike '%"+criteria+"%') and active = true");	 
			     } 
	...
	public Map<String, Object> getDataById(HashMap<String, Object> criteriaMap)
		...
		PIBSUtils.writeLogInfo(logger, MiscConstant.LOGGING_MESSSAGE_GETDATA_BY_ID);
		    ...
		    ProfessionalType model = (ProfessionalType) criteriaMap.get(MapConstant.CLASS_DATA);
			 ...
			 try {
				 ...
				 StringBuffer sqlLog = new StringBuffer();				
				 	...
				 	sqlLog.append("from pibs.file_professional_type ");
