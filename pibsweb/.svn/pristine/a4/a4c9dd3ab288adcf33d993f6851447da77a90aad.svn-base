...
public class SurgeryFormBean extends PIBSFormBean {
	...
	private List<ListValue> surgeryTypeLOV;
	...
	public void populateFormBean(Surgery model) {
		setId(model.getId());
		setSurgeryTypeId(model.getSurgeryTypeId());
		setDescription(model.getDescription());
		setRemarks(model.getRemarks());
		setFee(model.getFee()!=null ? model.getFee().toPlainString() : "0.00");
	}
	
	public Surgery populateModel (SurgeryFormBean formbean) {
		Surgery model = new Surgery();
		model.setId(formbean.getId());
		model.setSurgeryTypeId(formbean.getSurgeryTypeId());
	...
	public void populateDropdownList(HttpServletRequest request, SurgeryFormBean formBean) throws Exception{
        @SuppressWarnings("unchecked")
		List<ListValue> lovList = (ArrayList<ListValue>) request.getSession().getAttribute(MiscConstant.LOVTYPE_SURGERY_TYPE_SESSION);
    	if (lovList!=null) {
    		formBean.setSurgeryTypeLOV(lovList);
    	}
	}
		
	public void populateDropdownListFromSession(HttpServletRequest request, SurgeryFormBean formBean) throws Exception{
	...
	public void setSurgeryTypeId(int surgeryTypeId) {
		this.surgeryTypeId = surgeryTypeId;
	}

	public List<ListValue> getSurgeryTypeLOV() {
		return surgeryTypeLOV;
	}

	public void setSurgeryTypeLOV(List<ListValue> surgeryTypeLOV) {
		this.surgeryTypeLOV = surgeryTypeLOV;
	}
