...
public class MedicalSupplyFormBean extends PIBSFormBean {
	...
	private String retailSellingPrice;
	...
	public void populateFormBean(MedicalSupply model)  throws Exception  {
		...
		setRetailSellingPrice(model.getRetailSellingPrice()!=null ? model.getRetailSellingPrice().toPlainString() : "0.00");
	}
	...
	public MedicalSupply populateModel (MedicalSupplyFormBean formbean) throws Exception {
		...
		model.setRetailSellingPrice(formbean.getRetailSellingPrice()!=null && formbean.getRetailSellingPrice().trim().length()>0  ? new BigDecimal(formbean.getRetailSellingPrice()).setScale(2, BigDecimal.ROUND_HALF_UP) : new BigDecimal("0.00"));//2 decimal place
		formbean.setRetailSellingPrice(model.getRetailSellingPrice().toPlainString());
		return model;
	}
	...
	public ActionErrors validate(ActionMapping mapping,
		...
		if (command!=null && (command.equalsIgnoreCase(ParamConstant.AJAX_SAVE) || command.equalsIgnoreCase(ParamConstant.AJAX_UPDATE))) {
			...
			try {
				Double.parseDouble(this.getRetailSellingPrice()!=null && this.getRetailSellingPrice().trim().length()>0 ? this.getRetailSellingPrice() : "0.00");
			} catch (NumberFormatException e) {
				errors.add(ActionErrors.GLOBAL_MESSAGE, new ActionMessage("mf.price.num"));
				flag = true;
			}
	...
	public String getRetailSellingPrice() {
		return retailSellingPrice;
	}

	public void setRetailSellingPrice(String retailSellingPrice) {
