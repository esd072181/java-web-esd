...
public class MonitorEquipmentDaoImpl implements MonitorEquipmentDao {
	
	private final static Logger logger = Logger.getLogger(MonitorEquipmentDaoImpl.class);
	...
	public Map<String, Object> save(HashMap<String, Object> dataMap) throws Exception{
		...
		MonitorEquipment model = (MonitorEquipment) dataMap.get(MapConstant.CLASS_DATA);
		...
		StringBuffer qry =  new StringBuffer("insert into pibs.tran_patient_equipment (");  		
				qry.append("patientcasesystemid ");
				qry.append(",equipmentid ");
				qry.append(",qty ");
		  		qry.append(",fee ");
		  		qry.append(",amount ");
		...
		PIBSUtils.writeLogDebug(logger, "SQL: "+qry.toString());
		  ...
		  try {
			  ...
			  pstmt.setInt(2, model.getEquipmentId());
			  ...
			  if (statusInt == 1) {
				  conn.commit();
				  System.out.println("Inserted into Monitor Equipment is table successfully..");
				  status = true;
			  }
		  } catch (Exception e) {
			  conn.rollback();
	...
	public Map<String, Object> update(HashMap<String, Object> dataMap) throws Exception{
		...
		StringBuffer qry =  new StringBuffer("update pibs.tran_patient_equipment set ");	
			qry.append(" equipmentid=? ");
			qry.append(" ,qty=? ");
			qry.append(" ,fee=? ");
			qry.append(" ,amount=? ");
		...
		PIBSUtils.writeLogDebug(logger, "SQL: "+qry.toString());
		 ...
		 try {
			...
			pstmt.setInt(1, model.getEquipmentId());
			...
			if (statusInt == 1) {
				conn.commit();
				System.out.println("Monitor Equipment record (id: " +model.getId()+") updated successfully..");
				status = true;
			}
		} catch (Exception e) {
			conn.rollback();
	...
	public Map<String, Object> search(HashMap<String, Object> criteriaMap) throws Exception{
		...
		PIBSUtils.writeLogInfo(logger, MiscConstant.LOGGING_MESSSAGE_SEARCH);
		 	...
		 	MonitorEquipment modelCriteria = (MonitorEquipment) criteriaMap.get(MapConstant.CLASS_DATA);
			 ...
			 List<MonitorEquipment> rsList = new ArrayList<MonitorEquipment>();
					  
			 try {
				 conn = ServerContext.getJDBCHandle();

				 StringBuffer sql = new StringBuffer();
				 		sql.append("select a.id,a.patientcasesystemid,a.equipmentid,a.qty,a.fee,a.amount,a.datetaken,a.timetaken,b.entityname,b.description ");
				 		sql.append(" from pibs.tran_patient_equipment a, pibs.file_equipment b ");
					 	sql.append(" where a.equipmentid = b.id ");
				...
				PIBSUtils.writeLogDebug(logger, "SQL: "+sql.toString());
				 ...
				 while(rs.next()) {
		    		 ...
		    		 model.setEquipmentId(rs.getInt(3));
		    		 ...
		    		 model.setItemName(rs.getString(9));
	...
	public Map<String, Object> getDataById(HashMap<String, Object> criteriaMap)
		...
		PIBSUtils.writeLogInfo(logger, MiscConstant.LOGGING_MESSSAGE_GETDATA_BY_ID);
		    ...
		    MonitorEquipment model = (MonitorEquipment) criteriaMap.get(MapConstant.CLASS_DATA);
	...
	public Map<String, Object> delete(HashMap<String, Object> dataMap) throws Exception{
		...
		PIBSUtils.writeLogDebug(logger, "SQL: "+qry.toString());
		 ...
		 try {
			...
			if (statusInt == 1) {
				conn.commit();
				System.out.println("Monitor Equipment record (id: " +model.getId()+") deleted successfully..");
				status = true;
			}
		} catch (Exception e) {
			conn.rollback();
