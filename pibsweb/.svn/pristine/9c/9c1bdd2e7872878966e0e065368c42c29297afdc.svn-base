...
public class MonitorSurgeryDaoImpl implements MonitorSurgeryDao {
	
	private final static Logger logger = Logger.getLogger(MonitorSurgeryDaoImpl.class);
	...
	public Map<String, Object> save(HashMap<String, Object> dataMap) throws Exception{
		...
		MonitorSurgery model = (MonitorSurgery) dataMap.get(MapConstant.CLASS_DATA);
		...
		StringBuffer qry =  new StringBuffer("insert into pibs.tran_patient_surgery (");  		
				qry.append("patientcasesystemid ");
				qry.append(",surgeryid ");
				qry.append(",qty ");
		  		qry.append(",fee ");
		  		qry.append(",amount ");
		...
		PIBSUtils.writeLogDebug(logger, "SQL: "+qry.toString());
		  ...
		  try {
			  ...
			  pstmt.setInt(2, model.getSurgeryId());
			  ...
			  if (statusInt == 1) {
				  conn.commit();
				  System.out.println("Inserted into Monitor Surgery is table successfully..");
				  status = true;
			  }
		  } catch (Exception e) {
			  conn.rollback();
	...
	public Map<String, Object> update(HashMap<String, Object> dataMap) throws Exception{
		...
		StringBuffer qry =  new StringBuffer("update pibs.tran_patient_surgery set ");	
			qry.append(" surgeryid=? ");
			qry.append(" ,qty=? ");
			qry.append(" ,fee=? ");
			qry.append(" ,amount=? ");
		...
		PIBSUtils.writeLogDebug(logger, "SQL: "+qry.toString());
		 ...
		 try {
			...
			pstmt.setInt(1, model.getSurgeryId());
			...
			if (statusInt == 1) {
				conn.commit();
				System.out.println("Monitor Surgery record (id: " +model.getId()+") updated successfully..");
				status = true;
			}
		} catch (Exception e) {
			conn.rollback();
	...
	public Map<String, Object> search(HashMap<String, Object> criteriaMap) throws Exception{
		...
		PIBSUtils.writeLogInfo(logger, MiscConstant.LOGGING_MESSSAGE_SEARCH);
		 	...
		 	MonitorSurgery modelCriteria = (MonitorSurgery) criteriaMap.get(MapConstant.CLASS_DATA);
			 ...
			 List<MonitorSurgery> rsList = new ArrayList<MonitorSurgery>();
					  
			 try {
				 conn = ServerContext.getJDBCHandle();

				 StringBuffer sql = new StringBuffer();
				 		sql.append("select a.id,a.patientcasesystemid,a.surgeryid,a.qty,a.fee,a.amount,a.datetaken,a.timetaken,c.listvalue as type,b.description ");
				 		sql.append(" from pibs.tran_patient_surgery a, pibs.file_surgery b, pibs.list_value c ");
					 	sql.append(" where a.surgeryid = b.id ");
					 	sql.append(" and b.surgerytypeid = c.id ");
				...
				PIBSUtils.writeLogDebug(logger, "SQL: "+sql.toString());
				 ...
				 while(rs.next()) {
		    		 ...
		    		 model.setSurgeryId(rs.getInt(3));
	...
	public Map<String, Object> getDataById(HashMap<String, Object> criteriaMap)
		...
		PIBSUtils.writeLogInfo(logger, MiscConstant.LOGGING_MESSSAGE_GETDATA_BY_ID);
		    ...
		    MonitorSurgery model = (MonitorSurgery) criteriaMap.get(MapConstant.CLASS_DATA);
	...
	public Map<String, Object> delete(HashMap<String, Object> dataMap) throws Exception{
		...
		PIBSUtils.writeLogDebug(logger, "SQL: "+qry.toString());
		 ...
		 try {
			...
			if (statusInt == 1) {
				conn.commit();
				System.out.println("Monitor Surgery record (id: " +model.getId()+") deleted successfully..");
				status = true;
			}
		} catch (Exception e) {
			conn.rollback();
