...
public class PatientModule implements PIBSModule {
	...
	private final static Logger logger = Logger.getLogger(PatientModule.class);
		...
		try {
			...
			switch(action) {
				...
				case ActionConstant.SEARCH_PATIENT_FOR_ADMISSION: returnMap = bo.getRecordsForAdmission(dataMap);
			    		break;
				case ActionConstant.SEARCH_PATIENT_ADMITTED: returnMap = bo.getAdmittedRecords(dataMap);
	    				break;
				case ActionConstant.SEARCH_ALL_PATIENT_ADMITTED: returnMap = bo.getAdmittedRecords(dataMap);
						break;
				case ActionConstant.UPDATE_TO_ADMITTED: returnMap = bo.updateToAdmitted(dataMap);
						break;
				case ActionConstant.UPDATE_TO_ACTIVE: returnMap = bo.updateToActive(dataMap);
						break;
