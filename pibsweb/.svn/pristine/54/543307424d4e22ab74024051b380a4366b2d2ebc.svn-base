...
public class UserDaoImpl implements UserDao {
		    ...
		    User model = (User) criteriaMap.get(MapConstant.CLASS_DATA);
			 ...
			 try {
				 ...
				 while(rs.next()) {
		    		 ...
		    		 model.setRole(rs.getString(5));
		    		 model.setUserName(rs.getString(6));
		    		 model.setPassword(EncryptUtil.decrypt(rs.getString(7)));
				 }				 
			 } catch (SQLException e) {
				 throw e;
			 } finally {
	...
	public Map<String, Object> getDataByUsernamePassword(HashMap<String, Object> criteriaMap)
			throws Exception {
		// TODO Auto-generated method stub
		PIBSUtils.writeLogInfo(logger, MiscConstant.LOGGING_MESSSAGE_GETDATA_BY_CRITERIA);
		    ...
		    User model = (User) criteriaMap.get(MapConstant.CLASS_DATA);
			 ...
			 try {
				 ...
				 StringBuffer sql = new StringBuffer();				
				 	sql.append("select id,username,password from  pibs.file_user ");
				 	sql.append("where id = ? ");
