...
import com.pibs.dao.RoomCategoryDao;
...
public class RoomCategoryDaoImpl implements RoomCategoryDao {
	...
	public Map<String, Object> save(HashMap<String, Object> dataMap) throws Exception{
		...
		RoomCategory model = (RoomCategory) dataMap.get(MapConstant.CLASS_DATA);
		...
		StringBuffer qry =  new StringBuffer("insert into pibs.file_room_category (");
		...
		StringBuffer qryLog =  new StringBuffer("insert into pibs.file_room_category (");
		...
		PIBSUtils.writeLogDebug(logger, "SQL: "+qryLog.toString());
		  ...
		  try {
			  ...
			  pstmt.setBigDecimal(3, model.getRate());
			  ...
			  if (statusInt == 1) {
				  conn.commit();
				  System.out.println("Inserted into Room Category table successfully..");
				  status = true;
			  }
		  } catch (Exception e) {
			  conn.rollback();
	...
	public Map<String, Object> update(HashMap<String, Object> dataMap) throws Exception{
		...
		StringBuffer qry =  new StringBuffer("update pibs.file_room_category set ");	
			qry.append(" description=? ");
			qry.append(" ,remarks=? ");
			qry.append(" ,rate=? ");
		...
		StringBuffer qryLog =  new StringBuffer("update pibs.file_room_category set ");	
			qryLog.append(" description="+model.getDescription());
			qryLog.append(" ,remarks="+model.getRemarks());
			qryLog.append(" ,rate="+model.getRate());
		...
		PIBSUtils.writeLogDebug(logger, "SQL: "+qryLog.toString());
		 ...
		 try {
			...
			if (statusInt == 1) {
				conn.commit();
				System.out.println("Room Category record (id: " +model.getId()+") updated successfully..");
				status = true;
			}
		} catch (Exception e) {
			conn.rollback();
	...
	public Map<String, Object> delete(HashMap<String, Object> dataMap) throws Exception{
		...
		PIBSUtils.writeLogDebug(logger, "SQL: "+qryLog.toString());
		 ...
		 try {
			...
			if (statusInt == 1) {
				conn.commit();
				System.out.println("Room Category record (id: " +model.getId()+") deleted successfully..");
				status = true;
			}
		} catch (Exception e) {
			conn.rollback();
	...
	public Map<String, Object> search(HashMap<String, Object> criteriaMap) throws Exception{
		...
		PIBSUtils.writeLogInfo(logger, MiscConstant.LOGGING_MESSSAGE_SEARCH);
			 ...
			 PreparedStatement pstmt = null;
			  ...
			  List<RoomCategory> rsList = new ArrayList<RoomCategory>();
			 ...
			 try {
				 ...
				 if (category.equals(ActionConstant.SEARCHALL)) {
					 sql = new StringBuffer("select id,description,remarks,rate,createdby,createdon,modifiedby,modifiedon,version,active ");
					 	sql.append(" from pibs.file_room_category ");
		     ...
		     try {
			     ...
			     if (category.equals(ActionConstant.SEARCHALL)) {
			    	 sqlCount = new StringBuffer("select count(*) from pibs.file_room_category where active = true");	 
			     }else {
			    	 sqlCount = new StringBuffer("select count(*) from pibs.file_room_category where  (description ilike '%"+criteria+"%' or remarks ilike '%"+criteria+"%') and active = true");	 
			     } 
	...
	public Map<String, Object> getDataById(HashMap<String, Object> criteriaMap)
		...
		PIBSUtils.writeLogInfo(logger, MiscConstant.LOGGING_MESSSAGE_GETDATA_BY_ID);
		    ...
		    RoomCategory model = (RoomCategory) criteriaMap.get(MapConstant.CLASS_DATA);
			 ...
			 try {
				 ...
				 StringBuffer sql = new StringBuffer();				
				 	sql.append("select id,description,remarks,rate,createdby,createdon,modifiedby,modifiedon,version,active ");
				 	sql.append("from pibs.file_room_category ");
				 	sql.append("where id = ?");

				 StringBuffer sqlLog = new StringBuffer();				
				 	sqlLog.append("select id,description,remarks,rate,createdby,createdon,modifiedby,modifiedon,version,active ");
				 	sqlLog.append("from pibs.file_room_category ");
	...
	public Map<String, Object> getInActiveData(HashMap<String, Object> criteriaMap) throws Exception {
		...
		PIBSUtils.writeLogInfo(logger, MiscConstant.LOGGING_MESSSAGE_GET_INACTIVE_DATA);
		 ...
		 int limit = (int) criteriaMap.get(MapConstant.PAGINATION_LIMIT);
			 ...
			 PreparedStatement pstmt = null;
		     Map<String, Object> returnMap = null;
			 List<RoomCategory> rsList = new ArrayList<>();
		     ...
		     try {
			    ...
			    sqlCount = new StringBuffer("select count(*) from pibs.file_room_category where active = false");	 
