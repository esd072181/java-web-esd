			...
			<table  class="table table-bordered table-striped table-condensed table-hover" style="width: 98%;">  
				...
				<logic:iterate name="specializationForm" property="modelList" type="com.pibs.model.Specialization" id="model">
					<tr>				 
						...
						<c:if test="${sessionScope.user_role_session=='Admin'}">
							<td align="center"><a href="#" onclick="javascript: deleteSpecialization('<bean:write name="model" property="id"/>', '${specializationForm.category}',${specializationForm.currentPage})">Delete</a></td>
						</c:if>
					</tr>
				</logic:iterate>
			</table>
		...
		<div style="position: relative; top: -15px; float: left;">
			<ul class="pager">
				<c:if test="${specializationForm.currentPage != 1 && specializationForm.noOfPages > 0}">
					<li><a href="#" onclick="getSpecialization(${specializationForm.currentPage - 1},'${specializationForm.category}');">Previous</a></li>
				</c:if>
				<c:if test="${specializationForm.currentPage lt specializationForm.noOfPages}">
				  	<li><a href="#" onclick="getSpecialization(${specializationForm.currentPage + 1},'${specializationForm.category}');">Next</a></li>
				</c:if>
			</ul>
		</div>
