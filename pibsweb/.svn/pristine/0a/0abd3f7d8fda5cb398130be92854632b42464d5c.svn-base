		...
		<h3>Surgery - Edit Record</h3>
			<html:form action="/surgery.do" styleId="idForm" >
				<fieldset>
					...
					<html:hidden property="id" value="${surgeryForm.id}"/>
					<table>
						<tr>
							<td><label  class="control-label top-spacing" for="roleId">Type</label></td>
							<td style="padding-left: 5px;">
								<html:select  name="surgeryForm" property="surgeryTypeId" styleId="surgeryTypeIdId" disabled="${surgeryForm.transactionStatus}">
						...
						<tr>
							...
							<td style="padding-left: 5px;">
								<html:text style="text-align: right;" styleId="feeId" property="fee" styleClass="input-xlarge"  size="20" maxlength="20" value="${surgeryForm.fee}" disabled="${surgeryForm.transactionStatus}"></html:text><br>
							</td>
						</tr>
						<tr height="15px"></tr>				
						<tr>
									...
									<div class="controls">
										<c:choose>
											<c:when test="${surgeryForm.transactionStatus == true}">
												<html:button property="btnClose" styleClass="btn btn-primary" style="width: 100px;" onclick="goToSurgery();" value="Close"></html:button>
											</c:when>
											<c:otherwise>
												<html:button property="btnUpdate" styleClass="btn btn-primary" style="width: 100px;" onclick="updateSurgery();" value="Update"></html:button>
												&nbsp;&nbsp;
												<html:button property="btnCancel" styleClass="btn btn-primary" style="width: 100px;" onclick="goToSurgery();" value="Cancel"></html:button>
											</c:otherwise>
										</c:choose>						
									</div>							
								</div>
