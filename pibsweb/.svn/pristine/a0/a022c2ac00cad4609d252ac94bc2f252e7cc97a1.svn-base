...
public class MonitorAdditionalServicesDaoImpl implements MonitorAdditionalServicesDao {
	
	private final static Logger logger = Logger.getLogger(MonitorAdditionalServicesDaoImpl.class);
	...
	public Map<String, Object> save(HashMap<String, Object> dataMap) throws Exception{
		...
		MonitorAdditionalServices model = (MonitorAdditionalServices) dataMap.get(MapConstant.CLASS_DATA);
		...
		StringBuffer qry =  new StringBuffer("insert into pibs.tran_patient_additional_services (");  		
				qry.append("patientcasesystemid ");
				qry.append(",additionalservicesid ");
				qry.append(",qty ");
		  		qry.append(",fee ");
		  		qry.append(",amount ");
		...
		PIBSUtils.writeLogDebug(logger, "SQL: "+qry.toString());
		  ...
		  try {
			  ...
			  pstmt.setInt(2, model.getAdditionalServicesId());
			  ...
			  if (statusInt == 1) {
				  conn.commit();
				  System.out.println("Inserted into Monitor AdditionalServices is table successfully..");
				  status = true;
			  }
		  } catch (Exception e) {
			  conn.rollback();
	...
	public Map<String, Object> update(HashMap<String, Object> dataMap) throws Exception{
		...
		StringBuffer qry =  new StringBuffer("update pibs.tran_patient_additional_services set ");	
			qry.append(" additionalservicesid=? ");
			qry.append(" ,qty=? ");
			qry.append(" ,fee=? ");
			qry.append(" ,amount=? ");
		...
		PIBSUtils.writeLogDebug(logger, "SQL: "+qry.toString());
		 ...
		 try {
			...
			pstmt.setInt(1, model.getAdditionalServicesId());
			...
			if (statusInt == 1) {
				conn.commit();
				System.out.println("Monitor AdditionalServices record (id: " +model.getId()+") updated successfully..");
				status = true;
			}
		} catch (Exception e) {
			conn.rollback();
	...
	public Map<String, Object> search(HashMap<String, Object> criteriaMap) throws Exception{
		...
		PIBSUtils.writeLogInfo(logger, MiscConstant.LOGGING_MESSSAGE_SEARCH);
		 	...
		 	MonitorAdditionalServices modelCriteria = (MonitorAdditionalServices) criteriaMap.get(MapConstant.CLASS_DATA);
			 ...
			 List<MonitorAdditionalServices> rsList = new ArrayList<MonitorAdditionalServices>();
					  
			 try {
				 conn = ServerContext.getJDBCHandle();

				 StringBuffer sql = new StringBuffer();
				 		sql.append("select a.id,a.patientcasesystemid,a.additionalservicesid,a.qty,a.fee,a.amount,a.datetaken,a.timetaken,c.description as category,b.description ");
				 		sql.append(" from pibs.tran_patient_additional_services a, pibs.file_additional_services b, pibs.file_additional_services_category c ");
					 	sql.append(" where a.additionalservicesid = b.id ");
					 	sql.append(" and b.additionalservicescategoryid = c.id ");
				...
				PIBSUtils.writeLogDebug(logger, "SQL: "+sql.toString());
				 ...
				 while(rs.next()) {
		    		 ...
		    		 model.setAdditionalServicesId(rs.getInt(3));
	...
	public Map<String, Object> getDataById(HashMap<String, Object> criteriaMap)
		...
		PIBSUtils.writeLogInfo(logger, MiscConstant.LOGGING_MESSSAGE_GETDATA_BY_ID);
		    ...
		    MonitorAdditionalServices model = (MonitorAdditionalServices) criteriaMap.get(MapConstant.CLASS_DATA);
	...
	public Map<String, Object> delete(HashMap<String, Object> dataMap) throws Exception{
		...
		PIBSUtils.writeLogDebug(logger, "SQL: "+qry.toString());
		 ...
		 try {
			...
			if (statusInt == 1) {
				conn.commit();
				System.out.println("Monitor AdditionalServices record (id: " +model.getId()+") deleted successfully..");
				status = true;
			}
		} catch (Exception e) {
			conn.rollback();
