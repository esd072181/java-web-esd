		...
		PIBSUtils.writeLogDebug(logger, "SQL: "+qry.toString());
		  ...
		  try {
			  ...
			  if (statusInt == 1) {
				...
				//update the available beds of roomid (current room) 
				...
				roomDataMap.put(MapConstant.ACTION, ActionConstant.UPDATE_AVAILABLE_BEDS_AFTER_PAYMENT_OR_ROOM_TRANSFER);
			    ...
			    if (resultMap!=null && !resultMap.isEmpty()) {
				    ...
				    if (transactionStatusforUpdate) {
						...
						//update the available beds of roomid transfer(new room) 
				    	roomModel = new Room();
						roomModel.setId(model.getRoomIdTransfer());
						roomDataMap = new HashMap<String, Object>();
						roomDataMap.put(MapConstant.MODULE, ModuleConstant.ROOM);
						roomDataMap.put(MapConstant.CLASS_DATA, roomModel);
						roomDataMap.put(MapConstant.USER_SESSION_DATA, user);
						roomDataMap.put(MapConstant.JDBC_CONNECTION, conn);
					    ...
					    if (resultMap!=null && !resultMap.isEmpty()) {
					        boolean transactionStatusforUpdateInner = (boolean) resultMap.get(MapConstant.TRANSACTION_STATUS);
						    if (transactionStatusforUpdateInner) {
								conn.commit();
		...
		PIBSUtils.writeLogInfo(logger, MiscConstant.LOGGING_MESSSAGE_GETDATA_BY_ID);
			...
			MonitorRoomTransfer model = (MonitorRoomTransfer) criteriaMap.get(MapConstant.CLASS_DATA);
			 ...
			 try {
				 ...
				 StringBuffer sql = new StringBuffer();				
				 	sql.append("select a.id,a.patientcasesystemid,a.roomid,a.dateadmitted,a.roomidtransfer,a.datetransferred,b.roomno as roomno, c.rate as rate,d.roomno as roomnotransfer, e.rate as ratetransfer,e.description as roomcategory ");
				 ...
				 while(rs.next()) {
					 ...
					 rsModel.setRoomCategory(rs.getString(11));
		    		 rsList.add(rsModel);
				 }				 
			 } catch (SQLException e) {
				 throw e;
		...
		PIBSUtils.writeLogDebug(logger, "SQL: "+qry.toString());
		 ...
		 try {
			...
			if (statusInt == 1) {
				...
				//update the available beds of roomid (current room)  - subtract
				Room roomModel = new Room();
				roomModel.setId(model.getRoomId());
				HashMap<String,Object> roomDataMap = new HashMap<String, Object>();
				roomDataMap.put(MapConstant.MODULE, ModuleConstant.ROOM);
			    ...
			    if (resultMap!=null && !resultMap.isEmpty()) {
				    ...
				    if (transactionStatusforUpdate) {
						...
						//update the available beds of roomid transfer(new room) - add 
				    	roomModel = new Room();
						roomModel.setId(model.getRoomIdTransfer());
						roomDataMap = new HashMap<String, Object>();
						roomDataMap.put(MapConstant.MODULE, ModuleConstant.ROOM);
