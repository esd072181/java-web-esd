...
$(function() {
    ...
    $("#idSearchCriteria").focus();
	  ...
	  <div>
	  	<label style="font-weight: bold; font-size: 20px;">Medical Supply</label>	
	  	<html:button property="btnAdd" styleClass="btn btn-primary btnMonitoringSmallest" onclick="goToMonitorMedicalSupplySearch('${monitorMedicalSupplyForm.patientCaseSystemId}');" value="Add New"></html:button>
	  	<span id="msgId" style="color: blue; padding-left: 100px; display: none;"><bean:message key="msg.updated"/></span>	  
	  </div>

	  <div class="table-responsive"  style="padding-top: 10px;" >
		<c:choose>
			<c:when test="${monitorMedicalSupplyForm.entityList != null}">
				<table  id="tableId" class="table table-bordered table-striped table-condensed table-hover context-menu-table" style="width: 98%;">  
					...
					<logic:iterate name="monitorMedicalSupplyForm" property="entityList" type="com.pibs.model.MonitorMedicalSupply" id="model" indexId="index">
						<tr>
							<td>${index+1}</td>
							<td><bean:write name="model" property="itemDescription"/></td>
							<td><bean:write name="model" property="itemGenericName"/></td>
							...
							<td align="center"><a href="#" onclick="javascript: editMonitorMedicalSupply('<bean:write name="model" property="id"/>');">Edit</a></td>
							<td align="center"><a href="#" onclick="javascript: deleteMonitorMedicalSupply('<bean:write name="model" property="id"/>','${monitorMedicalSupplyForm.patientCaseSystemId}');">Remove</a></td>
						</tr>
					</logic:iterate>
				</table>
			</c:when>
